...
@click.version_option(version=__version__)
def main() -> None:
    """Candy LFS - Git LFS server with GitHub authentication."""
    pass
...
def set_endpoint(url: str) -> None:
    ...
    Example: candy-lfs config set-endpoint https://api.example.com
    """
    config.api_endpoint = url
    console.print(f"[green]✓[/green] API endpoint set to: {url}")
...
@click.argument("tenant_id")
def login(tenant_id: str) -> None:
    """Login with GitHub for a tenant.
    ...
    try:
        ...
        # Set as current tenant if none selected
        if not config.current_tenant:
            config.current_tenant = tenant_id
...
@click.argument("tenant_id", required=False)
def logout(tenant_id: Optional[str]) -> None:
    """Logout from a tenant (or current tenant)."""
    if not tenant_id:
        tenant_id = config.current_tenant
        if not tenant_id:
            raise click.ClickException("No tenant specified and no current tenant selected")
...
def tenant_info(tenant_id: Optional[str]) -> None:
    ...
    try:
        ...
        info = client.get_tenant(tid)  # type: ignore
...
@repo.command("list")
@click.option("--tenant", "-t", help="Tenant ID (uses current if not specified)")
def list_repos(tenant: Optional[str]) -> None:
    """List repositories in current tenant."""
    try:
        client = get_client(tenant)
        tid = tenant or config.current_tenant
        
        repos = client.list_repos(tid)  # type: ignore
...
def create_repo(repo_name: str, tenant: Optional[str]) -> None:
    ...
    try:
        ...
        result = client.create_repo(tid, repo_name)  # type: ignore
...
def delete_repo(repo_name: str, tenant: Optional[str]) -> None:
    ...
    try:
        ...
        client.delete_repo(tid, repo_name)  # type: ignore
...
def list_tokens(tenant: Optional[str]) -> None:
    ...
    try:
        ...
        tokens = client.list_tokens(tid)  # type: ignore
...
@token.command("create")
@click.argument("name")
@click.option("--scope", type=click.Choice(["tenant", "repo"]), default="tenant", help="Token scope")
@click.option("--repo", help="Repository name (required if scope is 'repo')")
@click.option("--permission", type=click.Choice(["rw", "ro"]), default="rw", help="Permission level")
...
def create_token(
    ...
    try:
        ...
        console.print(f"[green]✓[/green] Token created: {result['token_id']}")
        console.print(f"\n[bold yellow]⚠  Save this token - it won't be shown again:[/bold yellow]")
        console.print(f"[bold cyan]{result['token']}[/bold cyan]\n")
...
def revoke_token(token_id: str, tenant: Optional[str]) -> None:
    ...
    try:
        ...
        client.delete_token(tid, token_id)  # type: ignore
...
def usage(tenant: Optional[str]) -> None:
    ...
    try:
        ...
        usage_data = client.get_usage(tid)  # type: ignore
...
# Fix the config group name conflict
main.add_command(config_cmd, name="config")
